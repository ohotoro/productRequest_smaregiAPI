    ...
    try {
      ...
      if (lastRow >= 7) {
        try {
          ...
          console.log('실제 데이터 행 수:', actualDataRows);
        } catch (e) {
  ...
  try {
    ...
    if (items.length > 0) {
      const data = items.map((item, idx) => {
        ...
        // fallback 로직 제거 - 정확한 인덱스 매칭만 사용
        const finalProtected = protected;
        ...
        return [
          ...
          item.priority || 3,               // H열
          item.comment || '',               // I열
          // J열: 상태 - 항상 현재 item의 상태 사용
          item.status || '대기',
          // K열: 확정시간 - 현재 item의 확정시간 사용
          item.confirmedAt || '',
          stockAvailable,                   // L열: 재고가능여부
          item.supplierName || '',          // M열
          // N열: 내보내기시간 - 확정된 항목만 보호된 데이터 유지
          (item.status === '확정' && finalProtected && finalProtected.exportedAt) || item.exportedAt || '',
          // O열: CSV확인 - 확정된 항목만 보호된 데이터 유지
          (item.status === '확정' && finalProtected && finalProtected.csvConfirmed) || (item.csvConfirmed ? '✓' : ''),
          // P열: 박스번호 - 확정된 항목만 보호된 데이터 유지
          (item.status === '확정' && finalProtected && finalProtected.boxNumbers) || item.boxNumbers || '',
      ...
      const fullData = data.map((row, index) => {
        ...
        // R열, S열 데이터 복원 - 확정된 항목만 보호 데이터 유지
        const itemStatus = row[9]; // J열의 상태값
        const rColumn = (itemStatus === '확정' && finalProtected && finalProtected.exportStatus) || '';
        const sColumn = (itemStatus === '확정' && finalProtected && finalProtected.shippedAt) || '';
...
function getOrderBoxNumber(orderId) {
  try {
    // 1. 먼저 PropertiesService에서 시도
    const scriptProperties = PropertiesService.getScriptProperties();
    const key = `boxNumber_${orderId}`;
    let boxNumber = scriptProperties.getProperty(key);
    
    // 2. Properties에 없으면 스프레드시트에서 마지막 박스번호 찾기
    if (!boxNumber) {
      const ss = SpreadsheetApp.openById(orderId);
      const sheet = ss.getSheetByName('발주서');
      const lastRow = sheet.getLastRow();
      ...
      if (lastRow > 6) {
        // P열(16번째 열)에서 박스번호 찾기
        const boxNumbers = sheet.getRange(7, 16, lastRow - 6, 1).getValues();
        let maxBoxNumber = 0;
        
        boxNumbers.forEach(row => {
          const boxStr = row[0];
          if (boxStr) {
            // "1(5), 2(3)" 형식에서 최대 박스번호 추출
            const matches = boxStr.match(/(\d+)\(/g);
            if (matches) {
              matches.forEach(match => {
                const num = parseInt(match.replace('(', ''));
                if (num > maxBoxNumber) maxBoxNumber = num;
              });
            }
          }
        });
